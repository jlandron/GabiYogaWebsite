...
async function getSmtpPassword(parameterName = '/gabi-yoga/workmail/smtp-password') {
  try {
    ...
    console.log('SSM Parameter result:', {
      parameterExists: !!result.Parameter,
      valueExists: !!(result.Parameter && result.Parameter.Value),
      valueLength: result.Parameter && result.Parameter.Value ? result.Parameter.Value.length : 0,
      parameterType: result.Parameter ? result.Parameter.Type : 'N/A'
    });
    ...
    if (!result.Parameter.Value) {
      throw new Error('Parameter value is empty or null');
    }
