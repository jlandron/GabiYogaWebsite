...
window.ScheduleEditor = class ScheduleEditor {
    ...
    populateClasses() {
        ...
        classes.forEach(classItem => {
            ...
            classBlock.querySelector('.copy-btn').addEventListener('click', (e) => {
                e.stopPropagation();
                // Create a deep copy of the class for use as a template
                const copiedClass = JSON.parse(JSON.stringify(classItem)); 
                
                // Remove identifying fields to ensure it's treated as a new class
                delete copiedClass.id;
                delete copiedClass.createdAt; 
                delete copiedClass.updatedAt;
                
                // Use schedule editor's own modal instead of the external class modal
    ...
    showModal(classData = null) {
        ...
        if (classData) {
            ...
            // Only set editId if the class has an ID (not for copied classes)
            if (classData.id) {
