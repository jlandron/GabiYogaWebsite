...
// Define the CloudFormation event and response types until @types/aws-lambda is installed
 ...
 */
async function verifyCrossRegion(domainName: string, sourceRegion: string, targetRegion: string): Promise<void> {
 ...
 */
async function syncDkimSettings(
  domainName: string, 
  sourceSES: AWS.SES, 
  targetSES: AWS.SES
): Promise<void> {
 ...
 */
async function syncMailFromDomain(
  domainName: string, 
  sourceSES: AWS.SES, 
  targetSES: AWS.SES
): Promise<void> {
