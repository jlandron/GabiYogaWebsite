...
/// Parallax Floating Flowers Effect
document.addEventListener('DOMContentLoaded', () => {
    // Get parallax section and floating flowers
    ...
    if (!parallaxSection || !floatingFlowers.length) return;
    
    // Function to update flower positions on scroll
    const updateFlowerPositions = () => {
        // Get scroll position
        const scrollTop = window.pageYOffset || document.documentElement.scrollTop;
        
        // Calculate position relative to the parallax section
        ...
        if (scrollTop + viewportHeight > sectionTop && scrollTop < sectionTop + sectionHeight) {
            // Calculate parallax value (0 to 1 range when section is in viewport)
            const parallaxValue = (scrollTop + viewportHeight - sectionTop) / (viewportHeight + sectionHeight);
            
            // Apply different parallax speeds to each flower
            floatingFlowers.forEach((flower, index) => {
                // Different speeds for different flowers
                const speedFactor = 0.6 + (index * 0.15); // 0.6, 0.75, 0.9, 1.05, 1.2, 1.35
                const yOffset = parallaxValue * 100 * speedFactor * (index % 2 === 0 ? -1 : 1);
                
                // Update transform with existing animations plus scroll effect
    ...
    window.addEventListener('scroll', updateFlowerPositions);
});
