 ...
 */
function generateCdnUrl(s3Key, prefix = 'gallery', context = {}) {
  ...
  // If the key starts with the prefix, remove it as CloudFront is already configured with that origin path
  const prefixWithSlash = `${prefix}/`;
  if (cleanKey.startsWith(prefixWithSlash)) {
    cleanKey = cleanKey.substring(prefixWithSlash.length);
  }
