...
function initAdminClassModal() {
    ...
    // Initialize class templates if available
    if (window.classTemplates && typeof window.classTemplates.init === 'function') {
        console.log('Initializing class templates');
        window.classTemplates.init();
    } else {
        console.log('Class templates not available');
    }
}
...
function setupModalListeners() {
    ...
    if (window.classTemplates) {
        ...
        applyTemplateBtn.addEventListener('click', () => {
            ...
            if (selectedTemplate) {
                window.classTemplates.apply(selectedTemplate);
            } else {
                showNotification('Please select a template first', 'warning');
            }
        });
    } else {
        // Hide the template section if templates aren't available
        const templateSection = modal.querySelector('.template-selector');
        if (templateSection) {
