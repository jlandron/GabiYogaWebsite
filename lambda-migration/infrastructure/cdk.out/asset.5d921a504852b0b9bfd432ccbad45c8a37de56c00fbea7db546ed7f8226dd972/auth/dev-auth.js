...
const ssm = new AWS.SSM({
  region: process.env.AWS_REGION || process.env.REGION || 'us-east-1' // Explicitly set region
});

// Log AWS SDK configuration for debugging
console.log('AWS SDK Configuration:', {
  region: ssm.config.region,
  credentials: !!ssm.config.credentials,
  endpointDiscoveryEnabled: ssm.config.endpointDiscoveryEnabled
});
...
/**
 * Get parameter from environment variables, SSM, or default values
 */
async function getParameter(parameterName) {
  try {
    console.log(`Getting parameter for: ${parameterName}`);
    
    // Check if we have the value in environment variables first
    if (parameterName === SSM_PASSWORD_PATH && process.env.DEV_PASSWORD) {
      console.log('Using password from environment variable');
      return process.env.DEV_PASSWORD;
    }
    
    if (parameterName === SSM_COOKIE_SECRET_PATH && process.env.DEV_COOKIE_SECRET) {
    ...
    if (now - paramCache.lastFetched < 5 * 60 * 1000) {
      ...
      if (parameterName === SSM_COOKIE_SECRET_PATH && paramCache.cookieSecret) {
        console.log('Using cached cookie secret');
        return paramCache.cookieSecret;
      }
    }
    
    // Fall back to default values - skip SSM for now until we debug the issue
    console.log(`Using default value for ${parameterName}`);
    ...
    /* Commented out SSM code until we debug the issue
    console.log(`Trying to fetch from SSM: ${parameterName}`);
    ...
    try {
      ...
      console.log(`Successfully fetched from SSM: ${parameterName}`);
      ...
      console.log(`Using default value for ${parameterName} after SSM error`);
      if (parameterName === SSM_PASSWORD_PATH) {
        return DEFAULT_PASSWORD;
      } else {
        return DEFAULT_COOKIE_SECRET;
