...
async function uploadOfferingImage(offeringId, file) {
    try {
        console.log('Uploading image for offering', offeringId, 'with file', file.name);
        ...
        const data = await response.json();
        console.log('Gallery upload API response:', data);
        ...
        console.log('Updating offering with image URL:', data.imageUrl);
        const updateResponse = await fetch(`/offerings/${offeringId}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${localStorage.getItem('token')}`
        ...
        if (!updateData.success) {
            throw new Error(updateData.message || 'Failed to update offering with image URL');
        }
        
        console.log('Successfully updated offering with image URL');
        return true;
    } catch (error) {
